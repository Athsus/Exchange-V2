...
import {CoinListClient} from "@manahippo/coin-list/src/client";
...
const aggListQuotes = async (fromSymbol: string, toSymbol: string, inputUiAmt: string) => {
  ...
  const xCoinInfo = agg.coinListClient.getCoinInfoByFullName(fromSymbol);
  const yCoinInfo = agg.coinListClient.getCoinInfoByFullName(toSymbol);
  const inputAmt = parseFloat(inputUiAmt);
  if(xCoinInfo == undefined || yCoinInfo == undefined){
      return;
  }
  const quotes = await agg.getQuotes(inputAmt, xCoinInfo, yCoinInfo);
  console.log(quotes[0].quote.outputUiAmt / quotes[0].quote.inputUiAmt)  // price
};
...
const test = async (fromSymbol: string, toSymbol: string, inputUiAmt: string) => {
